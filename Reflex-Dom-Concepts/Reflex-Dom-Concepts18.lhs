...
>   let eventsOldNew = attach eventsOld events
>   eventsOldNew'<- holdDyn (M.empty,M.empty) eventsOldNew
>   display eventsOldNew'
...
>   let valuesOld'' = Prelude.map (findWithDefault' (Left One) valuesOld') dragKeys'
>   let valuesOld''' = (constant valuesOld'') -- valuesOld''' :: Behavior t [Either Simple Misc]
>   let insertList = attachWith (zipWith (\v k -> (k,Insert v)) ) valuesOld''' dropKeys
>   let insertMap = fmap (fromList) insertList -- insertMap :: Event t (Map k (Construction Either Simple Misc))

>   insertMap' <- holdDyn M.empty insertMap
>   deleteMap' <- holdDyn M.empty deleteMap
...
> findWithDefault' :: Ord k => a -> Map k a -> k -> a
> findWithDefault' a m k = findWithDefault a k m
